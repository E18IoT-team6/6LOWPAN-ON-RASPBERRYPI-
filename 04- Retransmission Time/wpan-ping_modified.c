...
static int nl_msg_cb(struct nl_msg *msg, void *arg)
{
	...
	if (!attrs[NL802154_ATTR_SHORT_ADDR] || !attrs[NL802154_ATTR_PAN_ID] || !attrs[NL802154_ATTR_EXTENDED_ADDR])
		return NL_SKIP;
...
static int measure_roundtrip(struct config *conf, int sd)
{
	...
	if (conf->interval >= 1000)
	{ /* when interval is larger than 1s */
		double d_interval = (conf->interval * 1.0) / 1000;
		timeout.tv_sec = (int)d_interval;
		timeout.tv_usec = (int)((d_interval - timeout.tv_sec) * 1000000);
	}
	else
	{
		timeout.tv_sec = 0;
...
static void init_server(int sd)
{
	...
	// For measure the computing time
	struct timeval rcv_start_time, rcv_end_time;
	long usec = 0;
	...
	while (1)
	{
		gettimeofday(&rcv_start_time, NULL);	// Modified
		...
		if (buf[0] == NOT_A_6LOWPAN_FRAME)
		{
			...
			{
				gettimeofday(&rcv_end_time, NULL); // Modified

				usec = rcv_end_time.tv_sec - rcv_start_time.tv_sec; // Modified

				fprintf(stdout, "It took %.1f ns to resend \n", (float)usec); // Modified
			}
		}
	}
	free(buf);
